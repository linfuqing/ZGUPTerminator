...
**关键**：匹配时真人**不靠**收到 `Play` 来启动进关，真人 Host/Client 均由 UI 主动走 `ApplyStart`。ApplyStart 在启动协程前广播 `MatchStart`；`LoginManager.__Start` 仅在 `match==0` 的普通组队路径广播 `Play`。Host 登录循环在远端 Status 已等于本关时仍继续等待/接受 PlayerProperty 触发 Joined，避免 Status/PlayerProperty 到达顺序造成卡死。
