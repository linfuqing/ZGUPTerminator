...
| Bot 规模 | 启动时先按 `userID` 对 `botProfiles` 做非零唯一化（保留第一项，重复/零值告警并忽略），再按 `minBots`/`maxBots` 固定 spawn agent 数；**运行时动态扩缩尚未实现**（规划见落地方案 Phase 5） |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `botProfiles` | `BotProfile[]` | Bot 账号池；每项含 `userID`、`userName`、`userAvatar`、`power`。`userID` 同时是 Server identity 和 `userID → virtualPort` 路由键，必须非零且全池唯一；初始化在 Capture/Spawn 前确定性保留首项并忽略重复/零值项，禁止为同一 userID 建立两个 agent |
