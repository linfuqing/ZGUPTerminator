...
| 取消匹配 | 邀请入队前 Bot 须经 Server API 注入 `Mismatch` 清除 server `match`；Spawn 推迟空闲 `MatchToSend`（见 §8 / 架构约束 M4） |
...
Connect 注入完成只表示 `ConnectRequested`，并会清除上一会话的 `Connected` / live hello。只有收到本次连接真实 server hello 后才能设置 `Connected`；初始 Status(0)、注册与空闲匹配全部直接检查这个唯一门禁。一次性 `relayServerStatusInjected` 只说明业务 Status 已注入，不等于链路就绪。
