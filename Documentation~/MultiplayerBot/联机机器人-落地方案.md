...
**当前实现（2026-07-29）**：匹配的登录/进关描述已从 Host `Play` 和两张 Bot 配置表中解耦。双方在 `Register` 后调用 `LoginManager.ApplyStart`，由 ApplyStart 广播 `ClientMessageMatchStart(type=24)`，携带当前 `matchID`、服务器 `userStageID`、`isRestart/levelID/stage/levelName/sceneName`。Bot 只接受当前 matchID 的描述，先按该 tuple 选录像并发送登录 PlayerProperty（Status=0），下一 Tick 再发送 PlayerProperty + 非零 Status 并启动回放；匹配 Host/Joiner 使用同一路径。`LoginManager.__Start` 仅在 `match==0` 的普通组队发送 `Play`，匹配接收端仍保留 `match!=0` 防御忽略。原 `rankedMatchScenes` / `rankedMatchStageIDs` 及取模/段位映射代码已删除。L2 仍要求 Connect/Play/MatchStart 等显式类型不得进入 Invite canonicalize/评分。
